...
export class WasmSegmentProcessor {
    ...
    constructor(sample_rate, channels) {
        const ret = wasm.wasmsegmentprocessor_new(sample_rate, channels);
        this.__wbg_ptr = ret;
    ...
    process_interleaved(samples) {
        ...
        const ret = wasm.wasmsegmentprocessor_process_interleaved(this.__wbg_ptr, ptr0, len0, samples);
        return ret !== 0;
    }
    /**
    ...
    set_effect_config(config) {
        const ret = wasm.wasmsegmentprocessor_set_effect_config(this.__wbg_ptr, config);
        return ret !== 0;
    }
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('sakiot_dsp_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
