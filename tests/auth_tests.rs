...
fn test_access_token_encode_decode() {
    ...
    let encoded =
        Token::<Access>::encode(user_id, discord_token.clone(), &keys.access_encode).unwrap();
...
fn test_refresh_token_encode_decode() {
    ...
    let encoded =
        Token::<Refresh>::encode(user_id, discord_refresh_token.clone(), &keys.refresh_encode)
            .unwrap();
...
fn test_token_type_mismatch() {
    ...
    let encoded_access = Token::<Access>::encode(1, "t".into(), &keys.access_encode).unwrap();
